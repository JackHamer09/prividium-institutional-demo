...
const { createOffer, repoAddress } = useRepoContract();
...
const form = reactive({
  ...
  collateralToken: (defaultCollateralToken?.address || "") as Address | "",
...
const selectedLendToken = computed((): TokenConfig | undefined => {
  return tokens.find((t) => t.address === form.lendToken);
});
...
function resetForm() {
  ...
  // Keep default tokens
  form.lendToken = (defaultLendToken?.address || "") as Address | "";
  form.collateralToken = (defaultCollateralToken?.address || "") as Address | "";
...
async function handleSubmit() {
  ...
  const selectedCollateralToken = tokens.find((t) => t.address === form.collateralToken);
